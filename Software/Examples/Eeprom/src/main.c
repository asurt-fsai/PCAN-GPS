...
//! int main(void)
//------------------------------------------------------------------------------
//! @brief main function that is called at startup
//------------------------------------------------------------------------------
int main(void)
{
	...
	while(1) 	
	{
		...
		// catch ID 48Ah from CAN to save new data to the eeprom
		if (RxMsg.Id == 0x48A && RxMsg.Type == CAN_MSG_STANDARD) {
			// write new data to eeprom. Next power cycle will show a modified
			// greeting message.
			EEPROM_Write(EEPROM_INT, 0x100, &RxMsg.Data.Data8[0], 8);

			EEPROM_FlushCache(EEPROM_INT);
		} else {
