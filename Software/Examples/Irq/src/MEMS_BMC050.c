...
HWStatus_t MEMS_BMC050_init_Accelerometer(void){
	...
	AccIRQConfig.Reg_0x21.latch_int = 0x00;
	...
	/* wait some time before enabling interrupts */
	Wait_Usec(1000);
	...
	AccIRQConfig.Reg_0x17.data_en = 1;
	wAccReg(0x17, *((u8_t*)&AccIRQConfig.Reg_0x17));

	readCurrAccelerationValues(&AccDataRaw);
...
HWStatus_t MEMS_BMC050_init_Magnetometer(void){
	...
	wMagReg(0x4D, 0xFF); 	//<! 4Dh: generate Interrupts
	wMagReg(0x4E, 0x00); 	//<! 4Eh: each axis enable, line polarity don't care
	...
	readCurrMagValues(&CmpDataRaw);
	prcRawCmpValues(&CmpDataRaw, &BMC050_Readings);prcRawAccValues(&BMC050_dbg_data_acc, &BMC050_Readings);
...
//------------------------------------------------------------------------------
//! void MEMS_BMC050_ReadMagData(void)
//------------------------------------------------------------------------------
void MEMS_BMC050_ReadMagData(void){
	readCurrMagValues(&CmpDataRaw);
	prcRawCmpValues(&CmpDataRaw, &BMC050_Readings);prcRawAccValues(&BMC050_dbg_data_acc, &BMC050_Readings);
...
//------------------------------------------------------------------------------
//! void MEMS_BMC050_handleMagIRQ(void)
//------------------------------------------------------------------------------
void MEMS_BMC050_handleMagIRQ(void){
	u8_t mag_irq_status;

	rMagReg(0x4A, &mag_irq_status);

	if((mag_irq_status && 1<<0) == 1<<0){
	//	Low Int X

	}
	if((mag_irq_status && 1<<1) == 1<<1){
	//	Low Int Y

	}
	if((mag_irq_status && 1<<2) == 1<<2){
	//	Low Int Z

	}
	if((mag_irq_status && 1<<3) == 1<<3){
	//	High Int X

	}
	if((mag_irq_status && 1<<4) == 1<<4){
	//	High Int Y

	}
	if((mag_irq_status && 1<<5) == 1<<5){
	//	High Int Z

	}
	if((mag_irq_status && 1<<6) == 1<<6){
	//	Overflow

	}
	if((mag_irq_status && 1<<7) == 1<<7){
	//	Data overrun
...
//------------------------------------------------------------------------------
//! void MEMS_BMC050_handleAccIRQ(void)
//------------------------------------------------------------------------------
void MEMS_BMC050_handleAccIRQ(void) {
	u8_t wBuff[bufsize];
