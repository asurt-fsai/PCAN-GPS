...
#define SSP_CH	2	/* SSP channel to use (0:SSP0, 1:SSP1) */

#define	CCLK			120000000UL	/* cclk frequency [Hz] */
#define PCLK_SSP	120000000UL	/* PCLK frequency for SSP [Hz] */
/* 30MHz max -> only even presaclers are valid and 2 is too small */
#define SCLK_FAST	30000000UL	/* SCLK frequency under normal operation [Hz] */
#define	SCLK_SLOW	30000000UL	/* SCLK frequency under initialization [Hz] */

#define	INS			((LPC_GPIO2->PIN & _BV(1)))	/* Socket status (true:Inserted, false:Empty) */
...
#elif SSP_CH == 2
#define	SSPxDR		LPC_SSP2->DR
#define	SSPxSR		LPC_SSP2->SR
#define	SSPxCR0		LPC_SSP2->CR0
#define	SSPxCR1		LPC_SSP2->CR1
#define	SSPxCPSR	LPC_SSP2->CPSR
#define	CS_LOW()	{LPC_GPIO1->CLR = 1<<8;}	/* Set P0.6 low */
#define	CS_HIGH()	{LPC_GPIO1->SET = 1<<8;}	/* Set P0.6 high */
#define PCSSPx		20
#define	PCLKSSPx	PCLK_SSP1
#endif
...
#define FCLK_FAST() { SSPxCPSR = (PCLK_SSP / SCLK_FAST) & ~1UL; }
#define FCLK_SLOW() { SSPxCPSR = (PCLK_SSP / SCLK_SLOW) & ~1UL; }
...
{
	LPC_SC->PCONP |= 1 << PCSSPx;		/* Enable SSP module */
...
#elif SSP_CH == 2
	LPC_IOCON->P1_0  |= 0x4;		// SSP2_SCK	= uSDcard
	LPC_IOCON->P1_1  |= 0x4;		// SSP2_MOSI= uSDcard
	LPC_IOCON->P1_4  |= 0x4;		// SSP2_MISO= uSDcard
	LPC_IOCON->P1_8 = 0x0;
	LPC_GPIO1->DIR |= 1 << 8;
#endif
	CS_HIGH();					/* Set CS# high */
...
)
{
	...
	if (send_cmd(CMD0, 0) == 1) {			/* Put the card SPI/Idle state */
		Timer1 = 10000;						/* Initialization timeout = 1 sec */
...
// 				while (send_cmd(CMD58, 0) == 0 && !(ocr[0] & 0x80) ) {		/* Check CCS bit in the OCR */	
// 					for (n = 0; n < 4; n++) ocr[n] = xchg_spi(0xFF);
// 					ty = (ocr[0] & 0x40) ? CT_SD2 | CT_BLOCK : CT_SD2;	/* Card id SDv2 */
// 					wait_ready(1000);
// 				}
